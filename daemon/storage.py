#!/usr/bin/env python3
"""
storage.py - Persistent storage for WatchCat daily network budget and weekly history.
Tracks daily consumption, rolling 7-day history, and user quotas in JSON format.
"""
...
class WatchCatStorage:
    ...
    def _default_config(self) -> Dict[str, Any]:
        return {
            ...
            "interface": "auto",  # auto detect active default route
            "theme_mode": "auto"   # auto (KDE theme) or watchcat (cyber olive)
        }
    ...
    def _load_history(self):
        """
        Loads the history structure:
        {
          "days": {
             "YYYY-MM-DD": {"down_bytes": int, "up_bytes": int, "total_bytes": int, "peak_rate_bps": int}
          },
          "last_interface_counters": {
             "rx_bytes": int,
             "tx_bytes": int,
             "timestamp": float
          }
    ...
    def record_traffic_delta(self, rx_delta: int, tx_delta: int, current_rate_bps: int = 0):
    ...
    def get_weekly_breakdown(self) -> List[Dict[str, Any]]:
        ...
        Matching the UI table:
        DAY | DATE | DOWN | UP | TOTAL
        e.g. Saturday 05 Sep, Sunday 06 Sep ... Friday 11 Sep
        """
        result = []
